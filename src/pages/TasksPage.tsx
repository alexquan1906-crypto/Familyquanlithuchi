...
export default function TasksPage() {
  const [activeTab, setActiveTab] = useState<'tasks' | 'lunar'>('tasks');
  const { tasks, loading, fetchTasks, addTask, toggleTaskCompletion, deleteTask } = useTasks();
  ...
  const lunarToday = getLunarDateMock(today);
      ...
      <div className="flex bg-slate-200/50 p-1 rounded-2xl w-full">
        <button
          ...
          className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-sm transition-all ${
            activeTab === 'tasks' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'
          }`}
        >
          ✅ Nhiệm Vụ
        </button>
        <button
          ...
          className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-sm transition-all ${
            activeTab === 'lunar' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-500'
          }`}
        >
          🌕 Lịch Âm & Lễ
        </button>
      ...
      {activeTab === 'tasks' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-slate-800">Danh Sách Việc Cần Làm</h2>
          </div>
          
          <TaskList 
            tasks={tasks}
            loading={loading}
            onAdd={async (title, date) => addTask({ title, due_date: date, is_completed: false, priority: 'medium', category: 'task' })}
